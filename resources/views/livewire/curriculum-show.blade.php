<div class="mx-auto p-4 text-gray-800">
    ...
    <h2 class="font-bold mb-2">Students - Present - {{ $curriculum->presentStudents() }} | Absent
        {{ $curriculum->course->students()->count() - $curriculum->presentStudents() }}</h2>
                ...
                <td class="border px-4 py-2">
                    <div class="flex items-center justify-center gap-4">
                        @if ($student->is_present($curriculum->id))
                            Presented
                        @else
                            <button wire:click="attendance({{ $student->id }})"
                                class="py-2 px-3 bg-green-500 hover:bg-green-600 text-white rounded-sm">Present</button>
                        @endif
                    </div>
                </td>
            </tr>
